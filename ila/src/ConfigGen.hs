...
instance
  ...
  ilaProbe (signalInfos, tracing) (WithIlaConfig @_ @_ @_ toplevel triggerPoint bufferDepth predicates) =
   ...
   where
    ilaHash =
      writeSignalInfo toplevel bufferDepth (natToNum @period) (fromGenSignal <$> signalInfos) (snd <$> predicates)
...
writeSignalInfo ::
  ...
  -- | Buffer size
  SNat s ->
  -- | Frequency of the domain being measured
  Natural ->
  ...
  BitVector 32
writeSignalInfo !_toplevel !_bufSize !_frequency !_sigInfo !_triggerNames = 0
...
signalInfoBBF _ _ args _ = view tcCache >>= go
 where
  go tcm
    | [toplevel, _, period, sigInfo, triggerNames] <- lefts args
    ...
    , Just (SomeNat (Proxy :: Proxy s)) <- someNatVal s =
        mkBlackBox
          $ getGenIla @n @m @s
            toplevel
            (SNat @s)
            (round . periodToHz @Float $ coerceTermToType @Natural period)
            (getSigInfo sigInfo)
  ...
  getGenIla ::
    ...
    Vec m (String) ->
    GenIla
  getGenIla toplevel bufSize freq sigInfo triggerNames =
    GenIla
      { toplevel = coerceTermToType toplevel
      , bufferSize = snatToNum bufSize
      , frequency = fromIntegral freq
      , hash =
          fromIntegral
            $ hash
