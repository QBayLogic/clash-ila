...
this file with the `-c` flag. More information about this under the **ILA-CLI** section.
...
To enter the TUI, simply invoke `ila-cli` with the `tui` parameter. Required arguments are `-c` for
the automatically generated ILA configuration json file and `-p` for the port the FPGA is connected
to.

Example:

`ila-cli tui -p /dev/ttyUSB0 -c _build/02-hdl/ilaconf.json`
