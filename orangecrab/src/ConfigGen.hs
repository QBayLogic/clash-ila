...
data IlaConfig dom where
  Ila ::
    ...
    -- ^ The signal indicating if the ILA has been triggered
...
instance
  forall dom n a.
  LabeledSignals ((Vec n GenSignal, Signal dom a) -> WithIlaConfig dom -> IlaConfig dom)
  where
  ilaProbe ::
    (Vec n GenSignal, Signal dom a) ->
    WithIlaConfig dom ->
    IlaConfig dom
  ilaProbe (signalInfos, tracing) WithIlaConfig{toplevel, triggerPoint, bufferDepth, trigger, capture} =
    Ila
      { depth = bufferDepth
...
-- into a `Vec`.
instance
  forall dom a boo m n next.
  (m ~ n + 1, BitPack boo, LabeledSignals ((Vec m GenSignal, Signal dom (a, boo)) -> next)) =>
  LabeledSignals ((Vec n GenSignal, Signal dom a) -> (Signal dom boo, String) -> next)
  where
  ilaProbe ::
    (Vec n GenSignal, Signal dom a) ->
    (Signal dom boo, String) ->
    next
  ilaProbe (prevInfos, prevSignal) (newSignal, newName) =
    ilaProbe (prevInfos ++ (newInfo :> Nil), newBundled)
   where
    newInfo = GenSignal{name = newName, width = natToNum @(BitSize boo)}
...
-}
ilaConfig ::
  (BitPack a, LabeledSignals ((Vec 1 GenSignal, Signal dom ((), a)) -> t)) =>
  (Signal dom a, String) ->
  t
ilaConfig (s :: (Signal dom a, String)) =
  ilaProbe (Nil :: Vec 0 GenSignal, pure () :: Signal dom ()) s

foo :: forall dom. (HiddenClockResetEnable dom) => IlaConfig dom
foo = go
 where
  counter :: Signal dom (Unsigned 8)
  counter = register 0 $ counter + 1

  indicator :: Signal dom Bool
  indicator = pure True

  go = ilaConfig
    (counter, "8 bit value")
    (indicator, "indicator led light")
    (counter, "another one")
    WithIlaConfig
      { toplevel = "my design"
      , triggerPoint = 0
