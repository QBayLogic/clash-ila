...
data WithIlaConfig dom n where
  WithIlaConfig ::
    forall dom n.
    ( KnownNat n
    , 1 <= n) =>
...
instance
  forall dom n a depth.
  ( NFDataX a
  , BitPack a
  , 1 <= BitSize a `DivRU` 32) =>
  LabeledSignals ((Vec n GenSignal, Signal dom a) -> WithIlaConfig dom depth -> IlaConfig dom depth a)
  where
  ilaProbe ::
    (Vec n GenSignal, Signal dom a) ->
    WithIlaConfig dom depth ->
